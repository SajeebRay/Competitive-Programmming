...
void solve(){
  ll n, m, x, y, d;
  cin >> n >> m >> x >> y >> d;
  if(d >= abs(n-x)+abs(m-y)){
    cout << -1;
    return;
  }
  bool fal1 = false, fal2 = false;
  loop(i,1,n){
    if(d >= abs(i-x)+abs(1-y)){
      fal1 = true;
    }
    if(d >= abs(i-x)+abs(m-y)){
      fal2 = true;
    }
  }
  bool fal3 = false, fal4 = false;
  loop(i,1,m){
    if(d >= abs(n-x)+abs(i-y)){
      fal3 = true;
    }
    if(d >= abs(1-x)+abs(i-y)){
      fal4 = true;
    }
  }
  if((fal1 == false && fal3 == false) || (fal2 == false && fal4 == false))
    cout << (n-1)+(m-1);
  else cout << -1;
}
int main()
