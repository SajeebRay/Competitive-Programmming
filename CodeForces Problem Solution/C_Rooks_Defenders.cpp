...
////////////////////* Solution *///////////////////
void solve(){
  ll n, q, k, row, col, row2, col2;
  cin >> n >> q;
  ordered_set s_row, s_col;
  mi m_row, m_col;
  while(q--){
    cin >> k;
    if(k == 1) {
      ...
      m_row[row]--;
      m_col[col]--;
      if(m_row[row] == 0) s_row.erase(row);
      if(m_col[col] == 0) s_col.erase(col);
    }
    else{
      cin >> row >> col >> row2 >> col2;
      ll x, y;
      x = s_row.order_of_key(row2+1)- s_row.order_of_key(row);
      y = s_col.order_of_key(col2+1)-s_col.order_of_key(col);
      if(x == (row2-row)+1 || y == (col2-col)+1)
        cout << "Yes" << nl;
      else
        cout << "No" << nl;
    }
