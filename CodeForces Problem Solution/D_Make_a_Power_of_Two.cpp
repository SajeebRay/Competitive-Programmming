...
void solve(){
  ...
  loop(i,0,60){
    ll x = (1LL << i);
    string sample = to_string(n);
    string to_make = to_string(x);
    ll sam = sample.length();
    ll tom = to_make.length();
    ll s = 0, r = 0, rem, cnt = 0;
    while(r < tom){
      if(sample[s] == to_make[r]){
        r++;
      } else {
        ++cnt;
      }
