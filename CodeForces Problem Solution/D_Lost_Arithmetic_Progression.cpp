...
////////////////////* Solution *///////////////////
bool isPresent(ll val, ll start, ll d, ll n){
  ll x = val-start;
  if(x%d == 0 && val >= start && (x/d) < n) 
    return true;
  else 
    return false;
}
...
void solve(){
  ...
  ll cNext = cend + cd;
  ll cprev = c1 - cd;
  if(!isPresent(c1,b1,bd,bn) || !isPresent(cend, b1, bd, bn) || (cd%bd) != 0)
    cout <<0;
  else if(!isPresent(cprev,b1,bd,bn) || !isPresent(cNext, b1, bd, bn))
  cout << -1;
  else{
    ll ans = 0;
    vl divisor;
    ...
    for(auto d: divisor){
      ...
      ans += ((possible((cNext-cend), d) + 1) * (possible(c1-cprev, d) +1))%mod;
      ans %= mod;
    }
    cout << ans;
  }
