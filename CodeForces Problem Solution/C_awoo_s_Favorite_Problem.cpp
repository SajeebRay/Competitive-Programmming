...
void solve(){
  ...
  cin >> n;
  string s, t;
  vl s_aidx, s_cidx, t_aidx, t_cidx;
  cin >> s >> t;
  string s2, t2;
  loop(i,0,n){
    if(s[i]!='b') s2.pb(s[i]);
  ...
  loop(i,0,n){
    if(s[i] == 'a') s_aidx.pb(i);
    if(t[i] == 'a') t_aidx.pb(i);
  }
  loop(i,0,n){
    if(s[i] == 'c') s_cidx.pb(i);
    if(t[i] == 'c') t_cidx.pb(i);
  }
  loop(i,0,s_aidx.size()){
    if(s_aidx[i] > t_aidx[i]){
      cout << "NO";
      return;
    }
  }
  loop(i,0,s_cidx.size()){
