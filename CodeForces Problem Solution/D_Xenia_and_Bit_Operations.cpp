...
////////////////////* Solution *///////////////////
vector<ll> tree, arra;
void constract(ll node, ll begin, ll end, ll op){
  if(begin == end){
    tree[node] = arra[begin];
    return;
  }
  ll left = node*2;
  ll right = (node*2)+1;
  ll mid = begin + (end-begin)/2;
  constract(left, begin, mid, op^1);
  constract(right, mid+1, end, op^1);
  if(op == 1) 
    tree[node] = (tree[left] | tree[right]);
  else 
    tree[node] = (tree[left] ^ tree[right]);
}
void update(ll node, ll begin, ll end, ll i , ll value,ll op){
  if(i < begin || i > end)
    return;
  if(i == begin && end == i) {
    tree[node] = value;
  ...
  ll right = (node*2)+1;
  ll mid = begin + (end-begin)/2;
  update(left,begin,mid,i, value, (op^1));
  update(right, mid+1, end, i, value, (op^1));
  if(op == 1) 
    tree[node] = (tree[left] | tree[right]);
  else 
    tree[node] = (tree[left] ^ tree[right]);
