...
void solve(){
  ...
  if(equal == n-1){
    ...
    while(!ok){
      ll r = n-1, l = n-2;
      while(l >= 0){
        if(arra[r].ff == arra[l].ff){
          l--;
          continue;
        }
        if(arra[r].ff > arra[l].ff){
          if(arra[r].ff%arra[l].ff != 0){
            arra[r].ff = (arra[r].ff/arra[l].ff) + 1;
          }
          else{
            arra[r].ff = arra[r].ff/arra[l].ff;
          }
          ans.pb({arra[r].ss, arra[l].ss});
        }
        if(arra[r].ff < arra[l].ff){
          r--;
        }
      ...
      ll same = 0;
      loop(i,0,n-2){
        if(arra[i].ff == arra[i+1].ff) same++;
      }
      if(same == n-1) ok = true;
    }
    cout << ans.size() << nl;
    for(auto x: ans) cout << x.ff << " " << x.ss << nl;
  }
}
int main()
{
