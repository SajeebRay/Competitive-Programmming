...
void solve(){
  ...
  for(auto x: a){
    auto it = b.find(x);
    auto it2 = c.find(x);
    if(it == b.end() && it2 == c.end()) ans1 += 3;
    else if(it == b.end() && it2 != c.end()) ans1 += 1;
    else if(it != b.end() && it2 == c.end()) ans1 += 1;
  }
  for(auto x: b){
    auto it = a.find(x);
    auto it2 = c.find(x);
    if(it == a.end() && it2 == c.end()) ans2 += 3;
    else if(it == a.end() && it2 != c.end()) ans2 += 1;
    else if(it != a.end() && it2 == c.end()) ans2 += 1;
  }
  for(auto x: c){
    auto it = b.find(x);
    auto it2 = a.find(x);
    if(it == b.end() && it2 == a.end()) ans3 += 3;
    else if(it == b.end() && it2 != a.end()) ans3 += 1;
    else if(it != b.end() && it2 == a.end()) ans3 += 1;
  }
  cout << ans1 << " " << ans2 <<  " " << ans3;
}
