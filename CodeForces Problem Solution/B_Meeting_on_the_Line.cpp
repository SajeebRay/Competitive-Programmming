...
void solve(){
  ...
  ascending(vp);
  float lm = 0, rm = 0, mid;
  mid = (vp[n-1].ff+vp[0].ff)/2;
  cout << mid << " c " << vp[n-1].ff << " " << vp[0].ff <<  nl;

  vector<float> dis(n); 
  loop(i,0,n-1){
    dis[i] = abs(mid-vp[i].ff) + vp[i].ss;
  }
  ll i = 0;
  while(vp[i].ff <= mid){
    lm = max(dis[i],lm);
  ...
  if(lm > rm){
    mid = mid - (lm-rm)/2;
    if(mid < vp[0].ff) mid = vp[0].ff;
  }
  else{
    mid = mid + (rm-lm)/2;
    if(mid > vp[n-1].ff) mid = vp[n-1].ff;
  }
  cout << mid;
}
int main()
