...
void solve(){
    ...
    loop(i, 3, n) {
        if (a[i] > a[cur]) {
            c[cur].se = i - 1; //checks winners next element is less or greater than this
            cur = i;
            c[cur].fi = i - 1; // updating first element, preparing for else condition the the query
        }
    }
    ...
    while (q--) {
        int u, k, ans;
        cin >> u >> k;
        if (c[u].fi == 0) ans = 0; // no win
        else if (c[u].se == 0) ans =  max(0, k - c[u].fi + 1); // biggest elements
        else {
            int z = min(c[u].se, k + 1); // collecting if next element is less than current
            ans = max(0, z - c[u].fi);
        }
        cout << ans << "\n";
    }
