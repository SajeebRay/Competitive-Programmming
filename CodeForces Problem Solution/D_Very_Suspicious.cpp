...
////////////////////* Solution *///////////////////
ll triangle = 0, le = 0, ri = 0, horizontal = 0, maxn = 1e9;
vpl paire;
ll cnt = 1;
void calculate(){
  while(maxn > triangle){
    if(cnt%3 == 0){
      ...
      triangle += (ri+horizontal)*2;
      le++;
    }
    else {
      triangle += (le + horizontal)*2;
      ri++;
    }
    paire.pb({triangle,cnt});
    cnt++;
  }
}
void solve(){
  ...
  cin >> n;
  pl p = {n,-1};
  ll ans = lower_bound(paire.begin(), paire.end(), p)->ss;
  cout << ans;
}
int main()
{
