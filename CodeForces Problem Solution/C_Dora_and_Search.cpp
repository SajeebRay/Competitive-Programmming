...
void solve(){
  ...
  cin >> n;
  vl per(n);
  loop(i,0,n-1) cin >> per[i];
  if(n < 4){
    cout << -1;
  }
  else{
    bool lowfind = false, highfind = false;
    ll ans1, ans2 = 0;
    ll lf = false, hf = false;
    ll high = n-1, low = 0, l = 1, r = n;

    while(low < high){
      if(per[low] == l || per[high] == l){
        if(per[low] == l) low++;
        else high--;
        // cout << low << " " << high << nl;
        l++;
      ...
      if(per[low] != l && per[high] != l && per[low] != r && per[high] != r) break;
    }
    if((high - low) > 2) cout << low+1 << " " << high+1;
    else cout << -1;
  }
}
int main()
