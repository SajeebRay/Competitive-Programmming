...
void solve()
{
  ...
  pre[n-1] = vp[n-1].ss;
  r_loop(i,n-2,0){
    pre[i] = max(pre[i+1], vp[i].ss);
  }
  loop(i,0,n-1){
    if(i != n-1) ans = min(ans, abs(vp[i].ff - pre[i+1]));
    if(pre[i+1] >= vp[i].ff || s.empty()){ // first condition is for avoiding same shopping blog
      s.insert(vp[i].ss);
      continue;
    }
    auto curr = s.lower_bound(vp[i].ff);
    if(curr == s.end()){
      curr--;
      ans = min(ans, abs(*curr - vp[i].ff));
    }
    else {
      ans = min(ans, abs(*curr - vp[i].ff));
      if(curr != s.begin()){
