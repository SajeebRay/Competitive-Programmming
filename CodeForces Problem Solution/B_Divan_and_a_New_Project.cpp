...
void solve(){
  ...
  cin >> n;
  vl v(n); // long vector
  vpl p;  // long vector pair
  loop(i,0,n){
    ll x; 
    cin >> x; 
    p.pb(mp(x,i)); // input value with index
  }
  ascending(p); //vector sort
  ll time = 0, temp, idx = 1;
  // head quater 0 te rekhe, beshi bar visit korte hoy emon bari gula 
  // 0 er kache rakhsi... like -2 -1 0 1 2
  for(ll i = n-1; i >= 0; i -= 2){ // 0 er positive side
    temp = 2*idx;
    time += p[i].ff*temp;
    v[p[i].ss] = idx;
    idx++;
  ...
  idx = -1;
  for(ll i = n-2; i >= 0; i -= 2){ // 0 er negative side
    temp = 2*idx;
    time += abs(p[i].ff*temp);
    v[p[i].ss] = idx;
    idx--;
  }
  cout << time <<nl << 0 << " ";
