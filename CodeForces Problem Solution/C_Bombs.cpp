...
bool pairScnd_Element2(const pair<int, int> &a, const pair<int, int> &b) { if(a.ff == b.ff) a.ss > b.ss; return (a.ff < b.ff); }
...
void solve(){
  ...
  cout << cnt << nl;
  for(auto x: pos){
    if(x.ff > 0) cout << "1 " << abs(x.ff) << " R" << nl;
    if(x.ss > 0) cout << "1 " << abs(x.ss) << " U" << nl;
    if(x.ff < 0) cout << "1 " << abs(x.ff) << " L" << nl;
    if(x.ss < 0) cout << "1 " << abs(x.ss) << " D" << nl;
    cout << 2 << nl;
    if(-1*x.ff > 0) cout << "1 " << abs(x.ff) << " R" << nl;
    if(-1*x.ss > 0) cout << "1 " << abs(x.ss) << " U" << nl;
    if(-1*x.ff < 0) cout << "1 " << abs(x.ff) << " L" << nl;
    if(-1*x.ss < 0) cout << "1 " << abs(x.ss) << " D" << nl;
    cout << 3 << nl;
  }
