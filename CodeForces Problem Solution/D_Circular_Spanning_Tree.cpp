...
void solve(){
  ...
  string str; cin >> str;
  ll odd = 0;
  loop(i,0,n-1) if(str[i] == '1') odd++;
  if((odd&1) || odd == 0) {cout << "NO" << nl; return; }
  else cout << "YES" <<nl;
  vl ones;
  loop(i,0,n-1) if(str[i] == '1') ones.pb(i);
  loop(i,0,ones.size()-1){
    for(ll j = ones[i]; (j+1)%n != ones[(i+1)%ones.size()]; j++){
      //cout << (j+1)%n << " x " << ones[(i+1)%ones.size()]  << nl;
      cout << j%n + 1  << ' ' << (j+1)%n + 1 << nl;
    }
  }
  loop(i,0,ones.size()-1){
    cout << (ones[0]+n -1)%n +1 << " " << ones[i] << nl;
  }
